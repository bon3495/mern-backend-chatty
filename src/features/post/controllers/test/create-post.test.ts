...
describe('Create', () => {
  ...
  describe('post', () => {
    it('should send correct json response', async () => {
      ...
      await CreatePost.prototype.normal(req, res);
      ...
      expect(postQueue.addPostJob).toHaveBeenCalledWith('savePostToDB', {
  ...
  describe('postWithImage', () => {
    it('should throw an error if image is not available', () => {
      ...
      CreatePost.prototype.withImage(req, res).catch((error: CustomError) => {
    ...
    it('should send correct json response', async () => {
      ...
      await CreatePost.prototype.withImage(req, res);
      const createdPost = spy.mock.calls[0][0].createdPost;
      expect(postServer.socketIOPostObject.emit).toHaveBeenCalledWith(
        'add post',
        createdPost
